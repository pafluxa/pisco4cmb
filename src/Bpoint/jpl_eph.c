...
#include <errno.h>
#include "jpl_eph.h"
// #include "astrtime.h"  Commented out 09/03/02 -MKB-


// Dummy routine added to return an error -MKB- 09/03/02
...
int open_ephemeris (char *file_nm = 0)
{
    if (stream != NULL) {
      ...
      if (stream == NULL) {
        printf ("open_ephemeris: Cannot open file '%s', errno = %X \n", _file_name, errno);
        return 0;
      }
      long foffset = 0;
      if (fseek (stream, foffset, SEEK_SET)) {
        printf ("open_ephemeris: file seek error\n");
        return 0;
      }
...
{
    if (stream == 0) {
        printf ("get_posn_vel: ephemeris file not open\n");
        return 0;
    }
	...
	// ephemeris file
        start_offset = time_org - hdr.epoch1;
        start_offset += time_offset;
        //printf( "%lf %lf %lf %lf\n", hdr.epoch1, time_org, time_offset, hdr.epoch2 );
        if ((start_offset < 0.0) ||
	...
	// was wrong
        ...
        start_offset += time_offset;
	int safety = 5;
	while ((start_offset < 0.0) && (foffset >= ((long)sizeof(hdr) + rec_len)) &&
					(--safety > 0)) {
	    foffset -= rec_len;
	    if (fseek (stream, foffset, SEEK_SET)) {
