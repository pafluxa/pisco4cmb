...
#define NSAMPLES (12*NSIDE*NSIDE)
...
int main( void )
{
	...
	GpuConvolver* conv = new GpuConvolver( NSAMPLES );
	conv->set_beam( beam );
	conv->set_sky( sky, beam.get_rho_max() );
	conv->set_detector_angle(0.0);

	start  = std::chrono::high_resolution_clock::now();
	conv->exec_convolution( data, scan, sky, beam );
	finish = std::chrono::high_resolution_clock::now();

	elapsed = finish - start;
	std::cerr << "GPU Convolution took " << elapsed.count() << " sec\n";
	delete conv;
	
	Convolver* cconv = new Convolver( NSAMPLES, 1 );
	start  = std::chrono::high_resolution_clock::now();
	cconv->exec_convolution( data, scan, sky, beam );
	finish = std::chrono::high_resolution_clock::now();
