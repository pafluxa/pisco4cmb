...
import numpy
nside = 512
print(healpy.query_disc(nside, (0,0,1), numpy.radians(20)).size)
#94612
**/
#define NPIXELS_BEAM (94612)
...
int main(int argc, char** argv )
{
    ...
    Convolver cconv2(&scan, &sky, &beam);
    ...
    cconv.configure_execution();
    // setup detetctor angle arays
    float detectorAngle[1];
    float positionAngles[] = {0, 45, 90};
    ...
    cconv2.exec_convolution(projDets, psbDataA2, psbDataB2);
    stop = std::chrono::steady_clock::now();
    double elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(stop - start).count();
    std::cerr << "[INFO]: time taken to convolve beam and sky: " << elapsed << "ms" << std::endl;
    for(int i = 0; i < NSAMPLES; i++)
    {
        outdata << std::setprecision(10)
                << psbDataA[i] << " " <<
                   psbDataA2[i] << " " <<
                   psbDataB[i] << " " <<
                   psbDataB2[i] << std::endl;
    }
    outdata.close();
    free_everything();
    return 0;
...
void allocate_everything(void) 
{
    ...
    psbDataA2 = (float*)malloc(sizeof(float)*NSAMPLES);
    psbDataB = (float*)malloc(sizeof(float)*NSAMPLES);
    psbDataB2 = (float*)malloc(sizeof(float)*NSAMPLES);
}
